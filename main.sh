...
(apt upgrade -y >/dev/null 2>&1) & loading $! "Upgrading installed packages"
...
(apt install gem -y >/dev/null 2>&1) & loading $! "Installing gem"
(gem install lolcat -y >/dev/null 2>&1) & loading $! "Installing lolcat gem"
(apt install dos2unix -y >/dev/null 2>&1) & loading $! "Installing dos2unix"
(apt install python -y >/dev/null 2>&1) & loading $! "Installing python"
...
TOOLS_SETUP() {
    ...
    (apt update -y >/dev/null 2>&1) & loading $! "Updating package lists"
    (apt upgrade -y >/dev/null 2>&1) & loading $! "Upgrading packages"
    (apt dist-upgrade -y >/dev/null 2>&1) & loading $! "Running dist-upgrade"
    ...
    (systemctl enable chronyd chrony >/dev/null 2>&1 && \
     systemctl restart chronyd chrony >/dev/null 2>&1 && \
...
systemctl stary rc-rocal >/dev/null 2>&1
...
clear
    ...
    (wget -q -O /etc/haproxy/haproxy.cfg "${WORKING_LINK}configure/haproxy.cfg") & loading $! "Re-downloading haproxy.cfg"
    (wget -q -O /etc/nginx/conf.d/xray.conf "${WORKING_LINK}configure/xray.conf") & loading $! "Re-downloading xray.conf"
    (curl -s "${WORKING_LINK}configure/nginx.conf" > /etc/nginx/nginx.conf) & loading $! "Re-downloading nginx.conf"
}
